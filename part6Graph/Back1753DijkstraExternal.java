...
public class Back1753DijkstraExternal {
        ...
        while (!que.isEmpty()){
            ...
            if (!visited[now.getNum()]){
                ...
                for (Node next : A[now.getNum()]
                    ...
                    if (D[next.getNum()] > now.getWeight() + next.getWeight()){
                        // 다음 노드까지의 최단 거리는 현재 노드까지의 최단거리 + 다음 노드에 대한 가중치
                        D[next.getNum()] = now.getWeight() + next.getWeight();
                        que.add(new Node(next.getNum() , D[next.getNum()]));
                    }
                }
            }
        }
