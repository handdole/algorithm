    ...
    class Solution {
        public int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
            ...
            if("code".equals(sort_by)){
                dataList.stream().sorted(Comparator.comparing(Data::getCode)).collect(Collectors.toList());
            }
            if("date".equals(sort_by)){
                dataList.stream().sorted(Comparator.comparing(Data::getDate)).collect(Collectors.toList());
            }
            if("maximum".equals(sort_by)){
                dataList.stream().sorted(Comparator.comparing(Data::getMaximum)).collect(Collectors.toList());
            }
            if("remain".equals(sort_by)){
                dataList.stream().sorted(Comparator.comparing(Data::getRemain)).collect(Collectors.toList());
            }
